...
class TestSharedTopLevelCachedDirectoryBuilder(object):
    def test_basic_build(self, mock_cas_helper):
        with (
            ...
            builder = SharedTopLevelCachedDirectoryBuilder(
                cache_root, mock_cas_helper, filesystem, meter
            )
            builder.init()
    ...
    def test_basic_build_with_cache(self, mock_cas_helper):
        with (
            ...
            builder_1 = SharedTopLevelCachedDirectoryBuilder(
                cache_root_1, mock_cas_helper, filesystem, meter
            )
            builder_1.init()
            builder_2 = SharedTopLevelCachedDirectoryBuilder(
                cache_root_2, mock_cas_helper, filesystem, meter
            )
            builder_2.init()
            builder_1.build(
    ...
    #             local_root, cache_root, mock_cas_helper, filesystem, meter
    #         )
...
class TestCacheSizeLimitWithCopy:
    def test_current_size_bytes(self, mock_cas_helper):
        with (
            ...
            filesystem.init()
            builder = SharedTopLevelCachedDirectoryBuilder(
                cache_root, mock_cas_helper, filesystem, meter, copy_file=True
            )
            builder.init()
    ...
    def test_evict_required(self, mock_cas_helper):
        with (
            tempfile.TemporaryDirectory() as filesystem_root,
            tempfile.TemporaryDirectory() as local_root,
            tempfile.TemporaryDirectory() as cache_root,
            ...
            dir_index = 3
            digest, dir_ = input_list[dir_index]
            builder.build(digest, dir_, local_root)
            _assert_directory(test_data_list[dir_index], local_root)
...
class TestCacheSizeLimitWithHardlink:
    def test_current_size_bytes(self, mock_cas_helper):
        with (
            ...
            filesystem.init()
            builder = SharedTopLevelCachedDirectoryBuilder(
                cache_root, mock_cas_helper, filesystem, meter, copy_file=False
            )
            builder.init()
            for digest, dir_ in input_list:
                builder.build(digest, dir_, local_root)
            ...
            test_data_list = [
                {
                    ...
                    "dir_1": {
                        "file_1_1": b"a" * 5,
                        "file_1_2": b"b" * 5,
                        "dir_1_1": {"file_1_1_1": b"c" * 5},
                        "dir_1_2": {"file_1_2_1": b"d" * 5},
                    },
                },  # size_bytes: 20
                {
                    "dir_1": {
                        "file_1_1": b"e" * 5,
                        "file_1_2": b"f" * 5,
                        "dir_1_1": {"file_1_1_1": b"g" * 5},
                        "dir_1_2": {"file_1_1_1": b"h" * 5},
                    },
                },  # size_bytes: 20
                {
                    "file_1": b"a" * 100,
